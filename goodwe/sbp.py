...
class SBP(Inverter):
    """Class representing inverter of SBP family"""
    ...
    __sensors: Tuple[Sensor, ...] = (
        Voltage("vbv1", 0, "Bus Voltage", Kind.PV),  # modbus 0x500
        Current("ibv1", 2, "Bus Current", Kind.PV),
        Calculated("pbv1", 0, lambda data, _: round(read_voltage(data, 0) * read_current(data, 2)), "Bus Power", "W",
                   Kind.PV),
        Integer("xxx3", 3, "Unknown sensor@3"),
        ...
        Integer("xx37", 37, "Unknown sensor@37"),
        Calculated("pgrid", 38,
                   lambda data, _: abs(read_power2(data, 38)) * (1 if read_byte(data, 80) == 2 else -1),
                   "On-grid Import/Export Power", "W", Kind.AC),
        ...
        Power("pload", 47, "On-grid Charge/Discharge Power", Kind.AC),
        ...
        Energy4("e_total", 59, "Total Battery Output", Kind.PV),
        Long("h_total", 63, "Hours Total", "h", Kind.PV),
        Energy("e_day", 67, "Today's Battery Output", Kind.PV),
